...
time_restart_ms = time.ticks_add(
    wlan_helper.time_start_ms, 3 * 60 * 60 * 1000
)  # after this time the board will restart and check for new files (max of about 298 days)
...
while True:
    ...
    if success == "UPLOAD_FAILED":
        continue
    if time.ticks_diff(time.ticks_ms(), time_restart_ms) > 0:
        wlan_helper.print_oled(
            "It is time to restart as the time period time_restart_ms is over."
        )
        wlan_helper.reset_after_delay()

    wlan_helper.time_next_update_ms = time.ticks_add(
        wlan_helper.time_next_update_ms, update_period_ms
    )

    while (
        time.ticks_diff(wlan_helper.time_next_update_ms, time.ticks_ms()) > 0
    ):  # we need to wait
        wlan_helper.feedWDT()
        wlan_helper.led.value(1)
        time.sleep_ms(50)
