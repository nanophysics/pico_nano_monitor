...
class Vibration:
    ...
    def process_isr(self, k):
        ...
        for i in range(self._listlength):  # measure multiple times
            self._sig_sig_measure = (
                self._vib_sig.read_u16() // 16
            )  # the RP Pico ADC has only 12 bit
        ...
        self._sig_abs_difference = abs(
            self._temp - self._difference_last
        )  # select the median, spikes will be removed this way
        self._difference_last = (
            self._temp
        )  # only take the difference to the last measurement into account, this way DC offset of the Amplifier will be removed
        expected_reference = 2250
        assert (
            self._sig_ref_measure < expected_reference + 1000
            and self._sig_ref_measure > expected_reference - 1000
        )  # Test Reference and therefore circuit
    ...
    def getPeakAverage(
        self,
    ):  # value between 0 and 1, peak since lasst reset, arbitrary unit
    ...
    def __init__(self, id_tags, pin="GPIO1"):
        ...
        if self.sensors:
            ...
            for s in self.sensors:
                utils.log.log(
                    "DS18b20 scanned sensor: '"
                    + "".join("%02X" % i for i in iter(s))
                    + "'"
                )
...
while True:
    ...
    if measurement_counter == 0 or not pico_tags.get(
        "vibration"
    ):  # vibration depends on measurement duration. Skip first measurement.
        pass
    else:
        utils.mmts.append(
            {
