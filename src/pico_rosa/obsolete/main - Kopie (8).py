...
class Sceduler:
    def __init__(self):
        ...
        self.ticks_last_log_print_interval_ms = 1000
    ...
    def sleep_ms(self, sleep_ms=0): # Use this instead of time.sleep in order to get things done!
        ...
        while time.ticks_diff(ticks_sleep_end_ms, time.ticks_ms()) > 0: 
            if time.ticks_diff(time.ticks_ms(), self.ticks_last_time_manager_ms) > 0:
                ...
                self.ticks_last_time_manager_ms = time.ticks_add(self.ticks_last_time_manager_ms, self.ticks_last_time_manager_interval_ms)
                break
            if time.ticks_diff(time.ticks_ms(), self.ticks_last_upload_check_ms) > 0:
                upload_check()
                self.ticks_last_upload_check_ms = time.ticks_add(self.ticks_last_upload_check_ms, self.ticks_last_upload_check_interval_ms)
                break  
            if time.ticks_diff(time.ticks_ms(), self.ticks_last_log_print_ms) > 0:
                ...
                self.ticks_last_log_print_ms = time.ticks_add(self.ticks_last_log_print_ms, self.ticks_last_log_print_interval_ms)
                break
            time.sleep_us(500)
...
def generiere_outage(outage_ms=50):
    ...
    relais.off()
    sceduler.sleep_ms(sleep_ms = outage_ms)
    relais.on()
